...
[Robot Framework](../glossary#robot-framework) is a script-based interpreter for files that contain textual specifications.
...
- **Indentation based blocks**: Code blocks like test, [task](../glossary#task) or [keyword](../glossary#keyword) bodies are defined by indentation.
...
- **Mostly case-insensitive**: Most elements like keyword or [variable](../glossary#variable) names are case insensitive.
...
Example of [test cases](../glossary#test-case) with their keyword calls written in Robot Framework:
...
In Robot Framework, **Test Cases** (**Tests**) or **Tasks** are executable entities that serve a specific purpose and are organized into [suites](../glossary#suite).
...
Tests or [Tasks](../glossary#task) have a body made up of **keyword calls** and Robot Framework statements like **IF** or **VAR**, which represent the actions or steps executed during the test or task execution.
...
- Each directory, starting from the top-level directory (the one executed by Robot Framework), and any sub-directory that contains a `*.robot` [suite](../glossary#suite) file, is considered a **Suite** as well.
