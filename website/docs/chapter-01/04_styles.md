...
Specification styles define how tests or [tasks](../glossary#task) are structured, focusing on how actions and verifications are described.
...
Both styles can be mixed, even within the same [test](../glossary#test-case) or [task](../glossary#task), but it is strongly recommended to have separate styles for separate purposes and not mix them within the same body.
One practical solution would be to define acceptance [test cases](../glossary#test-case) that cover users' expectations in a declarative *Behavior-Driven Style*, while using [keywords](../glossary#keyword) that are implemented in an imperative *[Keyword](../glossary#keyword)-Driven style*.
...
your [automation script](../glossary#automation-script) and their step sequences,
...
[Keywords](../glossary#keyword) define the specific actions that must be executed in a particular order, similar to procedural programming.
The emphasis is on the **actions performed by the automation/tester**.

For example, in Robot Framework, a Keyword-Driven [test](../glossary#test-case) might look like:
```robotframework
*** Test Cases ***
...
It provides a straightforward representation of the [task](../glossary#task) flow, making it easy to understand what actions will be executed.

Separation of the executed step/keyword and its [arguments](../glossary#argument)/data with spaces improves the readability of tests or [tasks](../glossary#task).
...
Therefore there are no multiple layers of [keywords](../glossary#keyword) that can be logged into execution protocols.
...
or detailed and involve a larger number of keyword calls within a [test](../glossary#test-case) or [task](../glossary#task).
Additionally, this style is better suited for complex [tasks](../glossary#task) or handling complex data,
as it enables a clear separation between keyword names and their [argument](../glossary#argument) values.
...
and is a method where the [test data](../glossary#test-data) and expected results are
...
statically in the [suite files](../glossary#suite-file) is normally not applicable to [RPA](../glossary#robotic-process-automation).
...
In this style, a single [user keyword](../glossary#user-keyword), which contains the whole test logic or sequence of actions,
...
- **Efficiency**: Reduces the need to write redundant [test cases](../glossary#test-case) by reusing the same workflow with different data inputs.
