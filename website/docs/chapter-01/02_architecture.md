...
[Robot Framework](../glossary#robot-framework) is an open-source automation framework that allows you to build automation scripts for testing and [RPA](../glossary#robotic-process-automation) (Robotic Process Automation).
...
- **Definition Layer**: This layer contains the "Test Data" ([test cases](../glossary#test-case), tasks, [resource files](../glossary#resource-file) which include [user keywords](../glossary#user-keyword) and variables).
In Robot Framework, the test data is written using the defined syntax and contains keyword calls and [argument](../glossary#argument) values that make the [test case](../glossary#test-case) or [task](../glossary#task) definitions structured in [suites](../glossary#suite).
...
The execution layer is responsible for processing this execution model to execute the [library keywords](../glossary#library-keyword) with their argument values, logging results, and generating reports.
...
When writing tests|tasks or [keywords](../glossary#keyword), the editor supports the definition layer.
