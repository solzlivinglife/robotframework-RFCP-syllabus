...
As stated before, it is possible to organize imports and available [keywords](../glossary#keyword) in Robot Framework by using [Resource Files](../glossary#resource-file).
By default, all [keywords](../glossary#keyword) or [variables](../glossary#variable) created or imported in a [resource file](../glossary#resource-file) are available to those suites and files that are importing that higher-level [resource file](../glossary#resource-file).

This can lead to complex import hierarchies or the importing of libraries multiple times, which should be avoided.

Due to this mechanism, the number of keywords available to a suite can be quite large, and naming conflicts, especially with [keywords](../glossary#keyword) from third-party [keyword libraries](../glossary#[keyword](../glossary#keyword)-library), can occur. These conflicts need to be resolved.


Some [keyword libraries](../glossary#[keyword](../glossary#keyword)-library) have the option to be configured to change their behavior, which may also change the available keywords they offer.
...
Let's assume the following libraries and [resource files](../glossary#resource-file) shall be used:
...
In this case, the suite `suite.robot` has access to all keywords from all [keyword libraries](../glossary#[keyword](../glossary#keyword)-library), as well as all [variables](../glossary#variable) and [user keywords](../glossary#user-keyword) from all [resource files](../glossary#resource-file).
With this transitive importing it is possible to organize [user keywords](../glossary#user-keyword) and imports of libraries in a hierarchical way.
...
Therefore, the recommendation is to import libraries only in one [resource file](../glossary#resource-file) with one configuration and use that import file in all places where the library is needed to make its keywords available.
...
Library importing [arguments](../glossary#argument) are used in the same way as keyword calls with [arguments](../glossary#argument).
If possible, it is recommended to set the [arguments](../glossary#argument) as [named arguments](../glossary#named-argument) to make usage more readable and future-proof.
...
If a proper IDE is used, that can be detected, and users can be warned after they have created a duplicate [user keyword](../glossary#user-keyword) name.
