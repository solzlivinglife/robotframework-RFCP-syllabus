...
[Resource Files](../glossary#resource-file) in Robot Framework are used to store reusable [keywords](../glossary#keyword),
[variables](../glossary#variable), and organize imports of other [resource files](../glossary#resource-file) and libraries.
See [2.4.2 Resource Files](chapter-02/04_keyword_imports.md#242-resource-files) for an introduction to [Resource Files](../glossary#resource-file).

Resource Files are typically used in many suites to share common [keywords](../glossary#keyword) and [variables](../glossary#variable) across different tests|[tasks](../glossary#task).
Therefore, they should be designed to be modular, reusable, and maintainable.
[Keywords](../glossary#keyword) and [variables](../glossary#variable) defined in one [resource file](../glossary#resource-file) should therefore
...
or [test](../glossary#test-case) a specific part or dialog of an application could be stored together in one [resource file](../glossary#resource-file).

Resource files are imported using the `Resource` setting in the
`*** Settings ***` section so that the path to the [resource file](../glossary#resource-file)
...
In difference to [suite files](../glossary#suite-file), resource files do **not** allow the `*** Test Cases ***` or `*** Tasks ***` sections.
...
- `*** Keywords ***` to define [user keywords](../glossary#user-keyword).
