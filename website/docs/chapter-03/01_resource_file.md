...
:term[Resource Files]{term="Resource File"} in Robot Framework are used to store reusable :term[keywords]{term="Keyword"},
:term[variables]{term="Variable"}, and organize imports of other :term[resource files]{term="Resource File"} and libraries.
See [2.4.2 Resource Files](chapter-02/04_keyword_imports.md#242-resource-files) for an introduction to Resource Files.

Resource Files are typically used in many suites to share common :term[keywords]{term="Keyword"} and :term[variables]{term="Variable"} across different tests|:term[tasks]{term="Task"}.
Therefore, they should be designed to be modular, reusable, and maintainable.
Keywords and variables defined in one :term[resource file]{term="Resource File"} should therefore
...
For example all :term[user keywords]{term="User Keyword"} and variables that do control
or :term[test]{term="Test Case"} a specific part or dialog of an application could be stored together in one :term[resource file]{term="Resource File"}.

Resource files are imported using the `Resource` setting in the
`*** Settings ***` section so that the path to the resource file
is given as an :term[argument]{term="Argument"} to the setting.
...
In difference to :term[suite files]{term="Suite File"}, resource files do **not** allow the `*** Test Cases ***` or `*** Tasks ***` sections.
...
- `*** Settings ***` to import libraries and other resource files.
  ...
  - `Variables` to import :term[variable files]{term="Variable File"}. (Not part of this syllabus)
  ...
  - `Keyword Tags` to set tags for all keywords in the resource file.
    defining and using :term[Keyword]{term="Keyword"} tags is not part of this syllabus.
...
- `*** Keywords ***` to define :term[user keywords]{term="User Keyword"}.
