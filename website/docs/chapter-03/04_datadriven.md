...
The **Data-Driven Specification** style in Robot Framework separates [test](../glossary#test-case)|[task](../glossary#task) logic from data, enabling tests|[tasks](../glossary#task) to be executed with multiple data sets efficiently. This approach involves using a single higher-level [keyword](../glossary#keyword) to represent the entire workflow, while the [test data](../glossary#[test](../glossary#test-case)-data) is defined as rows of input and expected output values.
...
For each [test](../glossary#test-case)|[task](../glossary#task), a template [keyword](../glossary#keyword) can be defined that contains the workflow logic.

At the suite level, the `Test Template` or `Task Template` setting can be used to specify that [keyword](../glossary#keyword).
All tests|[tasks](../glossary#task) in the suite will reuse this keyword for execution with different data sets.

Alternatively, the `[Template]` setting can be used at the test|[task](../glossary#task) level.
The tests|[tasks](../glossary#task) would not have any other keyword calls but would instead define the data rows to be passed to the template keyword.
...
With this approach, it is possible to define different scenarios in the same [suite file](../glossary#suite-file), which can be useful for testing different aspects of the same functionality.
