...
Thus, it is possible to define one Suite Setup that is executed at the very start of the execution before any other :term[Suite Setup]{term="Suite Setup"}, :term[Test|Task Setup]{term="Test|Task Setup"}, and :term[Test]{term="Test Case"}|:term[Task]{term="Task"} is executed.
...
- **`*** Keywords ***` Section (optional)**:
  ...
  Define keywords that are available to the initialization file for Suite Setup, Suite Teardown, :term[Test Setup]{term="Test Setup"}, or :term[Test Teardown]{term="Test Teardown"}.
