...
This linear execution can lead to issues when a preceding [test](../glossary#test-case)|[task](../glossary#task) fails, potentially affecting subsequent tests|[tasks](../glossary#task) due to an unclean state of the system under [test](../glossary#test-case) or the automated environment.
To prevent such issues, Robot Framework provides the **Teardown** functionality, which can be defined at the suite, [test](../glossary#test-case)|[task](../glossary#task), or [keyword](../glossary#keyword) level.

As mentioned before, a failure resulting in a [keyword](../glossary#keyword) with the status `FAIL` will cause Robot Framework not to execute all subsequent [keywords](../glossary#keyword) of the current test|[task](../glossary#task).
These not-executed [keywords](../glossary#keyword) will receive the status `NOT RUN`.

A **Teardown** is a single [keyword](../glossary#keyword) call with potential [argument](../glossary#argument) values that is executed after the child suites, test|[tasks](../glossary#task), and [keywords](../glossary#keyword) have completed execution, regardless of the outcome, even if previously executed elements have failed.
...
**Key characteristics of [Suite Teardown](../glossary#suite-teardown):**
- [Suite Teardown](../glossary#suite-teardown) is a single keyword call with potential [argument](../glossary#argument) values.
...
**Key characteristics of [Test|Task Teardown](../glossary#test-teardown):**
- Test|Task Teardown is a single keyword call with potential [argument](../glossary#argument) values.
...
- Deleting [test data](../glossary#test-data) created during the test|task.
