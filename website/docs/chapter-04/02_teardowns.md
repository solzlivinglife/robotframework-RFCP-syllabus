...
This linear execution can lead to issues when a preceding test|:term[task]{term="Task"} fails, potentially affecting subsequent tests|tasks due to an unclean state of the system under :term[test]{term="Test Case"} or the automated environment.
To prevent such issues, Robot Framework provides the **Teardown** functionality, which can be defined at the suite, test|task, or :term[keyword]{term="Keyword"} level.

As mentioned before, a failure resulting in a keyword with the status `FAIL` will cause Robot Framework not to execute all subsequent :term[keywords]{term="Keyword"} of the current test|task.
