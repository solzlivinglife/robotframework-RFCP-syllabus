...
At a basic level, you can run `robot` by providing the path to a [suite file](../glossary#suite-file) or suite directory containing [suite files](../glossary#suite-file) as last [argument](../glossary#argument).
```plaintext
robot <path_to_root_suite>
```

In case of the [2.2.5 Example Suite File](chapter-02/02_suitefile_syntax.md#225-example-suite-file) where a single [suite file](../glossary#suite-file) named `TestSuite.robot` is stored in a directory `robot_files`, to execute the example [test](../glossary#test-case) suite the following command is used, if the current working directory of the terminal is the directory containing the `robot_files` directory:
```plaintext
> robot robot_files
```
...
then creates an [execution model](../glossary#execution-model) and finally, executes all suites with their [test cases](../glossary#test-case) from that model.
...
The `robot` command can optionally be configured with additional options to control the execution behavior, such as setting output formats, specifying specific tests to run, or controlling logging levels and many more. These options are [named arguments](../glossary#named-argument) that are passed to the `robot` command BEFORE the path to the [suite file](../glossary#suite-file) or directory. To learn more about these options, you can use the help of the `robot` command like: `robot --help`.
...
- **`log.html`**: A detailed log file that provides an HTML view of the execution, including [keyword](../glossary#keyword)-level details.
- **`report.html`**: A summary report that gives an overview of the execution results, including statistics of tests|[tasks](../glossary#task) executed, passed, and failed.

`log.html` and `report.html` are generated based on the information stored in `output.xml`.

A unique feature of Robot Framework is, that it logs each [keyword](../glossary#keyword) call and its [arguments](../glossary#argument) with its log outputs and timestamps, so that it is possible to have a very detailed view of the execution flow and the data that was used during the execution.
In case of a failure it is possible to see the exact keyword call that failed and the [arguments](../glossary#argument) that were used, which can be very helpful for debugging or reporting. Furthermore, you also get all passed [keywords](../glossary#keyword) and even the non‑executed [keywords](../glossary#keyword), allowing you to trace the whole execution flow.
...
On Suite, [Test Case](../glossary#[test](../glossary#test-case)-case), [Task](../glossary#task) and Keyword Level:
...
- **`NOT RUN`**: Refers to [keywords](../glossary#keyword) that were not executed during execution, e.g. due to previous failure or conditions.
...
**Composite elements** like suites (composed of tests|tasks), tests|[tasks](../glossary#task) (composed of keywords) and [User Keywords](../glossary#user-keyword) (composed of Library Keywords and Robot Framework statements) do define their status based on the status of their child elements.
...
E.g. in case of [User Keywords](../glossary#user-keyword) this means that if all keywords or Robot Framework language statements that were directly called by that [User Keyword](../glossary#user-keyword) were `PASS` the [User Keyword](../glossary#user-keyword) itself is considered `PASS`.
...
That means that a [composite element](../glossary#composite-element) like suite, [test](../glossary#test-case)|[task](../glossary#task) or [User Keyword](../glossary#user-keyword) may be `PASS` even if some of its deeper child elements are `FAIL`.
...
Therefore a failure typically distributes upwards through the hierarchy of elements until it reaches the [root suite](../glossary#root-suite).

A User Keyword is `FAIL` if one of its called Library Keywords is `FAIL`.
A test|[task](../glossary#task) is `FAIL` if one of its directly called Keywords is `FAIL`.
