...
Robot Framework analyzes all containing files and determines if they contain [test cases](../glossary#[test](../glossary#test-case)-case) or [tasks](../glossary#task). If they do, they are considered **Suite Files** or **Low-Level Suites**.
All directories that either directly or indirectly contain a [Suite File](../glossary#suite-file) are considered **Suites Directories** or **Higher-Level Suites**.
...
Robot Framework parses files with the extension `.robot` and searches for [test cases](../glossary#[test](../glossary#test-case)-case) or [tasks](../glossary#task) within these files.

A parsed file that contains at least one [test case](../glossary#test-case) or [task](../glossary#task) is called a **Suite File**.

A [Suite File](../glossary#suite-file) **either** contains `*** Test Cases ***` (in [Test](../glossary#test-case) Suites) **or** `*** Tasks ***` (in [Task](../glossary#task) Suites), but it CANNOT contain both types simultaneously.
...
- `*** Keywords ***`- `*** Comments ***`The sections `*** Settings ***`, `*** Variables ***`, `*** Keywords ***`, and `*** Comments ***` are optional in [suite files](../glossary#suite-file) and can be omitted if not needed.
...
The `*** Settings ***` section is used to configure various aspects of the test|[task](../glossary#task) suite.
It allows you to import [keywords](../glossary#keyword) from external libraries (`Library`) or [resource files](../glossary#resource-file) (`Resource`), and import [variables](../glossary#variable) (`Variables`) from [variable files](../glossary#[variable](../glossary#variable)-file) (not part of this syllabus) that are needed for execution in the containing tests|[tasks](../glossary#task).
...
This section can also define [keywords](../glossary#keyword) called for execution flow control, such as `Suite Setup` and `Suite Teardown`, which are executed before and after the suite's tests run. See [4.1 Setups (Suite, Test|Task, Keyword)](chapter-04/01_setups.md) and
...
- `Test Setup`/`Task Setup` (locally: `[Setup]`) and `Test Teardown`/`Task Teardown` (locally `[Teardown]`) define which [keywords](../glossary#keyword) are executed before and after each individual test|task. The local setting overrides the suite's default. See [4.1 Setups (Suite, Test|Task, Keyword)](chapter-04/01_setups.md) and
[4.2 Teardowns (Suite, Test|Task, Keyword)](chapter-04/02_teardowns.md) for more information.
...
- `Test Template`/`Task Template` (locally `[Template]`) defines a template [keyword](../glossary#keyword) that defines the test|task body and is typically used for Data-Driven Testing where each test has the same keywords but different [argument](../glossary#argument) data. The local setting overrides the suite's default.

Similar to test|task tags, also [keyword](../glossary#keyword) tags can be defined in the `*** Settings ***` section with the `Keyword Tags` (locally `[Tags]`) setting, which can be used to set [keyword](../glossary#keyword) tags to the keywords. The local setting appends or removes tags defined by the suite's default.
...
This section is used to define [suite variables](../glossary#suite-variable) that are used in the suite or its tests|tasks or inside their keywords.

The most common use case is to define these [variables](../glossary#variable) as constants that contain a static value during execution.
This can either be a default value, that may be overwritten by globally defined [variables](../glossary#variable) via the [Command Line Interface](../glossary#command-line-interface) (CLI) or a constant value that is used in multiple places in the suite.
...
[Test cases](../glossary#test-case) and tasks are technically synonyms for each other.
However, users have to choose one of the two modes of suite execution that Robot Framework offers.

Each [test case](../glossary#test-case) or task is structured using an indentation-based format. The first un-indented line specifies the name of the test|task, followed by indented lines containing **keyword calls** and their possible **arguments** and test|task-specific settings.
...
A [suite file](../glossary#suite-file) must contain either a `*** Test Cases ***` or a `*** Tasks ***` section, but not both. [Resource files](../glossary#resource-file) cannot contain either of them.
...
while keywords defined in resource files can be used in any suite that imports these [resource files](../glossary#resource-file).
...
This section is used to add comments to the suite file or [resource file](../glossary#resource-file).
