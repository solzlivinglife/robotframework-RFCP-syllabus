...
[Suite files](../glossary#suite-file) and [resource files](../glossary#resource-file) share the same syntax, however they differ in their capabilities.
[Resource files](../glossary#resource-file) are explained in more detail in [2.4.2 Resource Files](chapter-02/04_keyword_imports.md#242-resource-files) [3.1 Resource File Structure](chapter-03/01_resource_file.md).
...
As mentioned before, Robot Framework uses an indentation-based and space-separated syntax to structure [keywords](../glossary#keyword), [test cases](../glossary#[test](../glossary#test-case)-case), and [tasks](../glossary#task).
...
**Example 1**: A [keyword](../glossary#keyword) call is a statement that consists of a [keyword](../glossary#keyword) name and its [arguments](../glossary#argument), which are separated by two or more spaces from the keyword name and from each other.
...
The first keyword call `Keyword Call` has two [arguments](../glossary#argument), `argument one` and `argument two`.
...
So, settings in the `*** Settings ***` section, [test cases](../glossary#[test](../glossary#test-case)-case) in the `*** Test Cases ***` section,
and [keywords](../glossary#keyword) in the `*** Keywords ***` section are written without any indentation.
However, when defining tests|[tasks](../glossary#task) and [keywords](../glossary#keyword), indentation is used to define their body, while their name is still un-indented,
e.g., after a [test case](../glossary#[test](../glossary#test-case)-case) name, all subsequent lines that are part of the [test case](../glossary#test-case) body are indented by two or more spaces.

That means that a body statement always starts with a separator token, followed by a data token, like e.g. [variable](../glossary#variable) or keyword as seen in the examples above.
...
Within the body of tests|[tasks](../glossary#task) and keywords, [control structures](../glossary#control-structure) like loops or conditions can be used. Their content should be indented by additional four spaces to make it clear that they are part of the [control structure](../glossary#control-structure). However, this is not mandatory and only a recommendation to make the file more readable.
...
A line continuation can only be performed where a separator is expected, like between a keyword name and its arguments or between two [arguments](../glossary#argument) or between a setting and its value(s).
...
If an [argument](../glossary#argument) value or any other token shall start with a hash (`#`)
...
- [variables](../glossary#variable) that are started by e.g. `${` (See [3.2 Variables](chapter-03/02_variables.md))
...
If values shall contain leading or trailing spaces they must be either enclosed in backslashes `\` or replaced by the special [variable](../glossary#variable) `${SPACE}` that contains a single space character.
...
In the following example, two [test cases](../glossary#test-case) are defined in a [suite file](../glossary#suite-file).
...
In this test, the first called keyword is `Connect To Server` without any arguments, while the second called keyword is `Login User`, and it has two [argument](../glossary#argument) values: `ironman` and `1234567890`.
...
The following tests will be executed in the order they are defined in the [suite file](../glossary#suite-file). First, the `Login User With Password` test case will be executed, followed by the `Denied Login With Wrong Password` test case.

Example [Suite File](../glossary#suite-file) Content:
