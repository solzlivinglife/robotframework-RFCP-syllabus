...
[Library Keywords](../glossary#library-keyword) and [User Keywords](../glossary#user-keyword) that are defined in a [resource file](../glossary#resource-file) should have a documentation text that describes what the [keyword](../glossary#keyword) does and how it should be used.

[Robot Framework](../glossary#robot-framework) is capable of generating **Keyword Documentation** files that contains a library- or resource-documentation, all [keywords](../glossary#keyword), their [argument](../glossary#argument) interfaces, and their documentation texts.
...
This keyword has 2 "[Mandatory Arguments](../glossary#mandatory-argument)" and 6 "[Optional Arguments](../glossary#optional-argument)".
...
The latter two [arguments](../glossary#argument) are optional.

The argument `arguments` is a "[Variable Number of Positional Arguments](../glossary#[variable](../glossary#variable)-number-of-positional-arguments)" and can only be set by position.
...
The argument `groups` is a "Variable Number of [Positional Arguments](../glossary#positional-argument)" and can only be set by position.
Therefore, if it shall be set, all preceding arguments must be set by position as well.
See [2.5.2.5 Variable Number of Positional Arguments](chapter-02/05_keyword_interface.md#2525-variable-number-of-positional-arguments) for more information about this kind of argument.
...
Also "Variable Number of Positional Arguments", represented with a single star (`*`) prefix, and "[Free Named Arguments](../glossary#free-named-argument)", represented with a double star (`**`) prefix are optional arguments.
...
- `"${url}" is open`- `the user clicks the "${button}" button`- `the page title should be ${exp_title}`- `the url should be ${exp_url}`
...
all arguments that are positioned before "Variable Number of Positional Arguments", "[Named-Only Arguments](../glossary#named-only-argument)", or "Free [Named Arguments](../glossary#named-argument)" in the argument interface of a keyword are "[Positional or Named Arguments](../glossary#positional-or-named-argument)".
...
When calling this keyword, the first [positional argument](../glossary#positional-argument) is assigned to `command`, while all subsequent positional arguments are collected into `arguments`.
...
The example of the `Run Process` keyword also has a free [named argument](../glossary#named-argument) `** configuration`.
