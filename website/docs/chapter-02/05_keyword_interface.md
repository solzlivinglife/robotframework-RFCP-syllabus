...
Library :term[Keywords]{term="Keyword"} and :term[User Keywords]{term="User Keyword"} that are defined in a :term[resource file]{term="Resource File"} should have a documentation text that describes what the :term[keyword]{term="Keyword"} does and how it should be used.

Robot Framework is capable of generating **Keyword Documentation** files that contains a library- or resource-documentation, all :term[keywords]{term="Keyword"}, their :term[argument interfaces]{term="Argument Interface"}, and their documentation texts.
...
Basically all standard and external 3rd party libraries offer these :term[Keyword]{term="Keyword"} Documentations as online available HTML pages.
...
The Keyword Documentation is structured so that it contains first the library or resource documentation, followed by a list of all keywords that are available in that library or :term[resource file]{term="Resource File"}.
...
- **Arguments** (opt.): The :term[argument interface]{term="Argument Interface"} that the keyword expects/offers its types and default values.
...
![Should Be Equal Keyword Documentation](/img/Should_Be_Equal_Docs.png)This keyword has 2 ":term[Mandatory Arguments]{term="Mandatory Argument"}" and 6 ":term[Optional Arguments]{term="Optional Argument"}".
...
The latter two :term[arguments]{term="Argument"} are optional and can be used depending on the style of defining keys and values the user likes most.

The :term[argument]{term="Argument"} `key_value_pairs` is a :term[Variable Number of Positional Arguments] and can only be set by position.
Therefore, if it shall be set, all preceding :term[arguments]{term="Argument"} must be set by position as well.
See [2.5.2.5 Variable Number of Positional Arguments](chapter-02/05_keyword_interface.md#2525-variable-number-of-positional-arguments) for more information about this kind of :term[argument]{term="Argument"}.
...
![Get Regexp Matches Keyword Documentation](/img/Get_Regexp_Matches_Docs.png)This keyword has 2 ":term[Mandatory Arguments]{term="Mandatory Argument"}" that can be called positionally or by name.
...
These arguments have to be before arguments with default values in the :term[argument interface]{term="Argument Interface"} of the keywords.
...
The first :term[Test]{term="Test Case"} will pass, because both argument values are equal.
The second :term[Test]{term="Test Case"} will fail, because the argument values are not equal.
...
Also :term[Variable Number of Positional Arguments], represented with a single star (`*`) prefix, and :term[Free Named Arguments]{term="Free Named Argument"}, represented with a double star (`**`) prefix are :term[optional arguments]{term="Optional Argument"}.
...
Keywords can include arguments embedded directly into their names, a feature primarily used for :term[Behavior-Driven Development]{term="Behavior-Driven Development"} (BDD).
:term[Embedded arguments]{term="Embedded Argument"} are mandatory and must be provided in the exact position defined within the keyword name.

Keyword names include arguments defined using the :term[scalar variable]{term="Scalar Variable"} syntax with dollar and curly braces (`${var_name}`).
...
- `"${url}" is open`- `the user clicks the "${button}" button`- `the page title should be ${exp_title}`- `the url should be ${exp_url}`Example :term[Test Case]{term="Test Case"}:
```robotframework
*** Test Cases ***
...
```
The optional prefixes `Given`, `When`, `Then`, `And` and `But` are basically ignored by Robot Framework if a keyword is found matching the rest of the name including the :term[embedded arguments]{term="Embedded Argument"}.
...
A mix of embedded arguments and "normal" arguments is possible to fully support :term[BDD]{term="Behavior-Driven Development"}.
In the keyword documentation the embedded arguments are written in :term[variable]{term="Variable"} syntax with dollar-curly-braces (`${var_name}`) to indicate that they are not part of the keyword name but are arguments.
...
Some keywords need to collect a :term[variable]{term="Variable"} amount of values into one argument, because it is not possible to define the amount of values in advance.
...
When calling this keyword, the first :term[positional argument]{term="Positional Argument"} is assigned to `dictionary`, while all subsequent :term[positional arguments]{term="Positional Argument"} are collected into `key_value_pairs`.
Because of this behavior, no additional :term[positional arguments]{term="Positional Argument"} can be used after these :term[Variable Number of Positional Arguments].
As a result, any arguments following these :term[Variable Number of Positional Arguments] must be :term[named arguments]{term="Named Argument"},
...
In this case all values given to the keyword as arguments, that do contain an unescaped equal sign (`=`) are considered as :term[named arguments]{term="Named Argument"}.

:term[Free named arguments]{term="Free Named Argument"} are marked with two asterisks `**` before the argument name in the keyword documentation.

The example of the `Set To Dictionary` keyword also has a :term[free named argument]{term="Free Named Argument"} `** items`.
...
It is important to know that keywords without a :term[return type hint]{term="Return Type Hint"} are often still returning values!
...
:term[User Keywords]{term="User Keyword"} typically have less extensive documentation because they are used in a narrower context and cannot be configured with arguments as much as library keywords from generic external libraries.
