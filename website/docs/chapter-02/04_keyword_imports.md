...
<!-- To use Keywords that are not part of BuiltIn, which is always imported invisibly, you must import keywords into the current scope. Basically Two different sources of [keywords](../glossary#keyword).
- Libraries, which contains low-level keywords actually implementing functionality, typically in Python.
- Resource Files, which either again import libraries or other [Resource Files](../glossary#resource-file) or they specify [User Keywords](../glossary#user-keyword).  -->
[Robot Framework](../glossary#robot-framework) has a modular design that allows users to import keywords from external sources.
Without importing external keywords into a [suite](../glossary#suite), only the keywords from Robot Framework's BuiltIn library are available for use, due to them being imported automatically.
...
As mentioned before resource files are used to organize and store keywords and [variables](../glossary#variable) that are used in multiple [suites](../glossary#suite).

They share a similar structure and the same syntax as suite files, but they do not contain [test cases](../glossary#test-case) or [tasks](../glossary#task).
